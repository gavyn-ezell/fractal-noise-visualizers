...
func perlin1d(x:float) -> float:
	#1. find the surrounding integers
	var floor_x= int(floor(x))
	var ceil_x = floor_x + 1

	#2. get the noise values at these surrounding integers
	var floor_noise = _hash1d(floor_x)
	var ceil_noise = _hash1d(ceil_x)

	#3. get the interpolation factor
	var t = x - floor_x

	#4. modify the interpolation factor with a fade function for smoother interpolation
	t = _fade1d(t)
	# 5. interpolate and return
	return lerp(floor_noise, ceil_noise, t)
	
func fractal1d(x: float) -> float:
	var final_noise = 0
	for s in range(octaves):
		...
		final_noise += pow(persistence, float(s)) * perlin1d(base_frequency * pow(lacunarity, float(s)) * x)
	return final_noise
